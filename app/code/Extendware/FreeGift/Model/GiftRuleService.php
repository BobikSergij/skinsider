...
use Extendware\FreeGift\Api\Data\GiftRuleDataInterfaceFactory;
...
class GiftRuleService implements GiftRuleServiceInterface
{
    ...
    public function getAvailableGifts(Quote $quote)
    {
        ...
        // if(GiftRuleDataInterface::NUMBER_OFFERED_PRODUCT !== 'undefined' ){
    ...
    public function replaceGiftProducts(Quote $quote, array $products, string $identifier, int $giftRuleId = null)
    {
        ...
        $totalQtyLimit = $giftRuleData[GiftRuleCacheHelper::DATA_NUMBER_OFFERED_PRODUCT];
        $totalQty = 0;
        foreach ($products as $product) {
            $totalQty +=  $product['qty'];
        }
        if ($totalQty > $totalQtyLimit) {
            throw new Exception(__('We can\'t add this gift item to your shopping cart. You can add maximum %1 qty.', $totalQtyLimit));
        }
        foreach ($products as $product) {
            if (!(isset($product['id']) && isset($product['qty']))) {
                throw new Exception(__('We found an invalid request for adding gift product.'));
